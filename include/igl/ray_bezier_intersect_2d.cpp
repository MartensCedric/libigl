  ...
  typename DerivedC::Scalar tol)
{
  ...
  // Evaluate point (via igl::cubic) and analytic derivative at t.
  auto eval_and_tangent = [&C](Scalar t, Vec2 & P, Vec2 & T)
  {
    igl::cubic(C, t, P);
    const Scalar s = Scalar(1) - t;
    T = Scalar(3) * (s * s * (C.row(1) - C.row(0)) +
  ...
  auto bezier_clip_1d = [](const Scalar d[4], Scalar & lo, Scalar & hi) -> bool
  {
    ...
    for (int i = 0; i < 4; ++i)
    {
      for (int j = i+1; j < 4; ++j)
      {
        const Scalar d0 = d[i];
        const Scalar d1 = d[j];
        const Scalar u0 = Scalar(u[i]);
        const Scalar u1 = Scalar(u[j]);
        if (d0 == d1)
        {
        ...
        if ((d0 > Scalar(0)) == (d1 > Scalar(0)) &&
          d0 != Scalar(0) && d1 != Scalar(0))
        {
          continue;
        }
        const Scalar t_cross = u0 + (u1 - u0) * (-d0) / (d1 - d0);
  ...
  while (!stack.empty())
  {
    ...
    const Scalar max_abs_d =
      std::max({std::abs(d[0]), std::abs(d[1]), std::abs(d[2]), std::abs(d[3])});
    const Scalar t_span = t_hi - t_lo;
    ...
    const bool all_pos =
      d[0] > Scalar(0) && d[1] > Scalar(0) && d[2] > Scalar(0) && d[3] > Scalar(0);
    const bool all_neg =
      d[0] < Scalar(0) && d[1] < Scalar(0) && d[2] < Scalar(0) && d[3] < Scalar(0);
    ...
    const Scalar new_span = u_hi - u_lo;
    ...
    if (new_span > Scalar(0.8) || depth >= max_depth / 2)
    {
      ...
      const Scalar t_mid = t_lo + Scalar(0.5) * t_span;
      stack.push_back({right, t_mid, t_hi, depth+1});
      stack.push_back({left, t_lo, t_mid, depth+1});
    }
    else
    {
      // Clip to [u_lo, u_hi].
      ...
      if (u_lo > Scalar(1e-10))
      {
        ...
        const Scalar t_split = (u_hi - u_lo) / (Scalar(1) - u_lo);
        CP4 cl, cr;
        split(rgt, t_split, cl, cr);
        clipped = cl;
      }
      else
      {
      ...
      const Scalar new_t_lo = t_lo + u_lo * t_span;
      const Scalar new_t_hi = t_lo + u_hi * t_span;
